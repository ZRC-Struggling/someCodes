myFocus.extend({//*********************淘宝商城风格******************
	mF_taobaomall:function(par){
		var box=this.$(par.id);
		this.addList(box,['txt']);this.wrap([this.$c('pic',box)],'cont')
		var cont=this.$c('cont',box),pics=this.$c('pic',box),txt=this.$li('txt',box);
		var n=txt.length,param={};//运行时相关参数
		pics.innerHTML+=pics.innerHTML;//无缝复制
		//CSS
		var pic=this.$li('pic',cont),dir=par.direction,dis=par.width;//先假设左右
		...
		this.$c('txt',box).style.width=2*par.width+'px';
		//PLAY
...
myFocus.setting.prototype.param('mF_taobaomall',{//额外参数设置
